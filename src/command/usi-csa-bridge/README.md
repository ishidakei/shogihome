...
# 対局用エンジンが予期せず終了したときにブリッジを終了するかどうか (オプショナル)
#   true              : 対局用の USI エンジンのプロセスが quit によらず終了した場合 (クラッシュや強制終了など)、
#                       エラーログを出力して終了コード 2 で直ちに終了します。systemd 等のスーパーバイザーでの
#                       再起動運用を想定しています。
#   false (省略時のデフォルト): 終了せず、エラーログを出力するのみで動作を継続します。
exitOnEngineUnexpectedClose: false
...
設定ファイルで `exitOnEngineUnexpectedClose: true` を指定すると、対局用の USI エンジンのプロセスが `quit` コマンドによらず終了した場合 (クラッシュや強制終了など)、ブリッジはエラーログを出力して終了コード 2 で直ちに終了します。 エンジンが死んだままブリッジがログインを維持すると、指し手を送れないまま時間切れになるためです。 systemd などのスーパーバイザーでブリッジを再起動する運用を想定しています。

この設定は既定では無効 (`false`) です。 指定しない場合は従来どおり、エラーログを出力するのみでブリッジは終了せず動作を継続します。
